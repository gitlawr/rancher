...
func SendMessage(notifier *v3.Notifier, recipient string, msg string) error {
	if notifier.Spec.SlackConfig != nil {
		if recipient == "" {
			recipient = notifier.Spec.SlackConfig.DefaultRecipient
		}
		return TestSlack(notifier.Spec.SlackConfig.URL, recipient, msg)
	}
	...
	if notifier.Spec.SMTPConfig != nil {
		...
		return TestEmail(s.Host, s.Password, s.Username, int(s.Port), s.TLS, msg, recipient, s.Sender)
	}
