...
func (l *Lifecycle) Sync(obj *v3.PipelineExecution) (*v3.PipelineExecution, error) {
	...
	if obj.Labels != nil && obj.Labels[utils.PipelineFinishLabel] == "true" {
		if err := l.cleanUpInstance(obj); err != nil {
			return obj, err
		}
	...
	//doIfOnCreation
	if err := l.setSharedEngine(obj); err != nil {
		return obj, err
	}
	if err := l.newExecutionUpdateLastRunState(obj); err != nil {
		return obj, err
...
func (l *Lifecycle) setSharedEngine(obj *v3.PipelineExecution) error {
	_, projectID := ref.Parse(obj.Spec.ProjectName)
	sharedSetting, err := l.pipelineSettingLister.Get(projectID, utils.SettingSharedEngine)
	if apierrors.IsNotFound(err) {
		return nil
	} else if err != nil {
		return err
	}
	shared := sharedSetting.Default
	if sharedSetting.Value != "" {
		shared = sharedSetting.Value
	}
	if shared == "true" {
		obj.Spec.SharedEngine = true
	}
	return nil
}
...
func (l *Lifecycle) cleanUpInstance(obj *v3.PipelineExecution) error {
	nsName := utils.GetPipelineCommonName(obj)

	jenkinsInstanceName := utils.JenkinsInstancePrefix + obj.Name
	if err := l.pods.DeleteNamespaced(nsName, jenkinsInstanceName, &metav1.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
		return errors.Wrapf(err, "Error delete Jenkins instance")
	}
	if err := l.services.DeleteNamespaced(nsName, jenkinsInstanceName, &metav1.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
		return errors.Wrapf(err, "Error delete Jenkins service")
	}
