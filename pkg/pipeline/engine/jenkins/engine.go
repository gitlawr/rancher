...
func (j *Engine) getJenkinsURL(execution *v3.PipelineExecution) (string, error) {
	...
	jenkinsInstanceName := utils.JenkinsInstancePrefix + execution.Name
	pod, err := j.PodLister.Get(ns, jenkinsInstanceName)
	if err != nil {
		return "", err
	}
...
func (j *Engine) PreCheck(execution *v3.PipelineExecution) (bool, error) {
	...
	if execution.Spec.SharedEngine {
		...
		pods, err := j.PodLister.List(utils.PipelineNamespace, set.AsSelector())
		if err != nil {
			return false, err
		}
		if len(pods) <= 0 {
		...
		ns := utils.GetPipelineCommonName(execution)
		podName := utils.JenkinsInstancePrefix + execution.Name
		pod, err = j.PodLister.Get(ns, podName)
		if err != nil {
			return false, err
		}
	}
