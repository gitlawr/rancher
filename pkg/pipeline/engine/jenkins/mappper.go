...
func ConvertPipelineExecutionToJenkinsPipeline(execution *v3.PipelineExecution) (PipelineJob, error) {
	...
	//logrus.Info(pipelineJob.Definition.Script)
...
func convertPipelineExecution(execution *v3.PipelineExecution) string {
	...
	if execution.Spec.SharedEngine {
		jenkinsURL = fmt.Sprintf("http://%s.%s:%d", utils.JenkinsName, utils.PipelineNamespace, utils.JenkinsPort)
	} else {
		jenkinsURL = fmt.Sprintf("http://%s:%d", utils.JenkinsInstancePrefix+execution.Name, utils.JenkinsPort)
	}
	return fmt.Sprintf(pipelineBlock, ns, containerbuffer.String(), images.Resolve(mv3.ToolsSystemImages.PipelineSystemImages.JenkinsJnlp), jenkinsURL, pipelinebuffer.String())
}
