...
MOCK_GITHUB_REPO_URL = 'http://localhost:5000/octocat/Hello-World.git'
MOCK_GITLAB_REPO_URL = 'http://localhost:5433/brightbox/puppet.git'
...
@pytest.mark.pipeline
def test_pipeline_stop_rerun(admin_pc, mock_github, remove_pipeline_namespace):
    client = admin_pc.client
    ...
    execution = client.by_id_pipeline_execution(execution.id)

    assert execution.executionState == 'Aborted'
...
def test_pipeline_run_expect_success(admin_pc, mock_github,
                                     remove_pipeline_namespace):
    client = admin_pc.client

    set_up_pipeline_github(admin_pc)

    pipeline = client.create_pipeline(repositoryUrl=EXAMPLE_REPO_URL,
                                      projectId=admin_pc.project.id)

    execution = pipeline.run(branch='master')
    remove_pipeline_namespace(admin_pc.project.id)

    wait_for_pipeline_execution_state(client, execution_id=execution.id)
...
def set_up_pipeline_github(admin_pc):
    ...
    config = {}
    config["hostname"] = "localhost:5000"
    config["tls"] = False
...
def set_up_pipeline_gitlab(admin_pc):
    ...
    config = {}
    config["hostname"] = "localhost:5433"
    config["tls"] = False
    config["clientId"] = "test_id"
    config["clientSecret"] = "test_secret"
