...
func NewCluster(driverName, name string, listenAddr string, configGetter ConfigGetter, persistStore PersistentStore) (*Cluster, error) {
	rpcClient, err := types.NewClient(driverName, listenAddr)
	if err != nil {
		return nil, err
	}
	return &Cluster{
